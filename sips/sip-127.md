...
* A new contract, `DebtPool`, will be deployed. Debt pools allow users to stake SNX and receive tokenised debt in the form of eSNX. Users can issue and burn a variety of assets supported by that debt pool - these are configurable via SCCP. 
...
* Fees that are generated from exchange of assets will continue being issued to the fee pool address. However, the mechanism of claiming those fees will be modified - users may claim fees using their eSNX, as a proportion of total staked SNX. 
...
* Tokenised debt:
  ...
  * eSNX would facilitate stakers in debt pools to be incentivised using a token streaming design.
